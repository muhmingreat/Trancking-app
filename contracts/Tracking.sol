...
mapping(address => Shipment[]) public shipments;
 ...
 struct TypeShipment {
  ...
  event ShipmentCreated(address indexed sender, 
    address indexed receiver, uint256 pickUpTime,
  ...
  event ShippmentDelivered (address indexed sender, 
  address indexed receiver, uint256 deliveryTime);
  ...
  function createShipment( address _receiver,
  uint256 _pickUpTime, uint256 _distance,
  uint256 _price) public payable {
    ...
    _pickUpTime, 0, _distance, _price, ShipmentStatus.PENDING, false );
  ...
  function startShipment(address _sender, address _receiver, uint256 _index) public {
    ...
    require(shipment.status == ShipmentStatus.PENDING, "Shippment already in transit");
    ...
    require(shipment.receiver == _receiver, "invalid receiver " );
    require(shipment.status == ShipmentStatus.IN_TRANSIT, "Shippment not in transit");
     require(!shipment.isPaid, "Shipment Alraedy paid");
     ...
     shipment.deliveryTime = block.timestamp; 
      ...
      uint256 amount = shipment.price;
       ...
       emit ShippmentDelivered(_sender, _receiver, shipment.deliveryTime );
